...
units.value = (route.query.units as "ml" | "oz" | "cl") || appState.defaultUnit;
...
onMounted(async () => {
    try {
        const resp = await BarAssistantClient.getPublicBarCocktail(barId, slug);
        if (resp?.data) {
            cocktail.value = mapPublicCocktailToPrintModel(resp.data);
        }
