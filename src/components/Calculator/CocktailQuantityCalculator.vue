...
const ingredientsWithCalculatedAmounts = computed((): IngredientWithCalculatedAmount[] => {
    return cocktails.value.flatMap(
        (cocktail) =>
...
const uniqueIngredients = computed(() => {
    const amountsByIngredients = ingredientsWithCalculatedAmounts.value.reduce(
        (accumulator: IngredientAccumulator, ingredient: IngredientWithCalculatedAmount): IngredientAccumulator => {
            ...
            return {
                ...
                prices: i?.prices
                    ?.map((p) => {
                        ...
                        return {
                            ...p,
                            amount: unitHandler.toFixedWithTruncate(unitHandler.convertFromTo(p.units, p.amount, selectedUnit.value), 2),
                            units: units,
                        };
                    })
                    .map((p) => {
        ...
        cocktails.value =
            ((
                await BarAssistantClient.getCocktails({
                    "filter[id]": collection.value?.cocktails?.map((c) => c.id).join(","),
                    include: "ingredients.ingredient",
                })
            )?.data as CocktailWithCount[]) ?? [];
    } else {
        cocktails.value = [];
    }
    ...
    ingredients.value =
        (
            await BarAssistantClient.getIngredients({
                "filter[id]": uniqueIngredients.value.map((i) => i.id).join(","),
                include: "prices",
                per_page: uniqueIngredients.value.length,
            })
        )?.data ?? [];
                            ...
                            <tr v-for="ingredient in finalIngredients" :key="ingredient.id">
                                ...
                                <td style="text-align: right">
                                    <ToggleIngredientShoppingCart
                                        :ingredient="{ id: ingredient.id, name: ingredient.name, slug: ingredient.slug }"
                                        :model-value="shoppingList.map((l) => l.ingredient.id).includes(ingredient.id)"
                                    ></ToggleIngredientShoppingCart>
                                </td>
                            </tr>
                            <tr style="border-top-width: 3px">
                                ...
                                <td colspan="2">
                                    <template v-for="(total, curr) in totalsPerCurrency" :key="curr">
                                        <template v-if="total.min_total == total.max_total"> {{ unitHandler.formatPrice(total.min_total, curr.toString()) }}<br /> </template>
                                        <template v-else>
