...
function removeImage(image: ImageViewModel) {
    confirm.show(t("imageupload.delete-confirm"), {
        onResolved: (dialog: any) => {
            ...
            BarAssistantClient.deleteImage(image.id)
                .then(() => {
...
<template>
    <div
        class="block-container block-container--padded block-container--inset image-upload"
        :class="{ 'block-container--placeholder': isDragover }"
        @drop="handleDrop"
