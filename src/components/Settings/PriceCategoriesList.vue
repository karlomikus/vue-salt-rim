    ...
    <PageHeader>
        ...
        <template v-if="showCreateAction" #actions>
            <SaltRimDialog v-model="showDialog">
                <template #trigger>
                    <button type="button" class="button button--dark" @click.prevent="openDialog($t('prices.add-price-category'), null)">
                        {{ $t("prices.add-price-category") }}
                    </button>
                </template>
                <template #dialog>
...
function refreshCategories() {
    ...
    BarAssistantClient.getPriceCategories()
        .then((resp) => {
            if (!resp) {
                return;
            }
...
function deletePriceCategory(category: components["schemas"]["PriceCategory"]) {
    confirm.show(t("price.category-confirm-delete", { name: category.name }), {
        onResolved: (dialog: any) => {
            isLoading.value = true;
            dialog.close();
            BarAssistantClient.deletePriceCategory(category.id)
                .then(() => {
                    isLoading.value = false;
