    ...
    <PageHeader>
        ...
        <template #actions>
            <SaltRimDialog v-model="showDialog">
                <template #trigger>
                    <button type="button" class="button button--dark" @click.prevent="openDialog($t('cocktail-methods.add'), {} as CocktailMethod)">
                        {{ $t("cocktail-methods.add") }}
                    </button>
                </template>
                <template #dialog>
...
function deleteMethod(method: CocktailMethod) {
    confirm.show(t("cocktail-methods.confirm-delete", { name: method.name }), {
        onResolved: (dialog: any) => {
            isLoading.value = true;
            dialog.close();
            BarAssistantClient.deleteCocktailMethod(method.id)
                .then(() => {
                    isLoading.value = false;
