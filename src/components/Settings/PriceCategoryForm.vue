...
function submit() {
    ...
    if (category.value.id) {
        BarAssistantClient.updatePriceCategory(category.value.id, postData)
            .then((resp) => {
        ...
        BarAssistantClient.savePriceCategory(postData)
            .then((resp) => {
                isLoading.value = false;
    ...
    <form @submit.prevent="submit">
        ...
        <div class="form-group">
            ...
            <p class="form-input-hint">
                <a href="https://en.wikipedia.org/wiki/ISO_4217#List_of_ISO_4217_currency_codes" target="_blank">{{ $t("price.currency-format-notice") }}</a>
            </p>
        </div>
        <div class="form-group">
