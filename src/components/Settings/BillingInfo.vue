                        ...
                        <div class="form-group">
                            <div class="billing__price-categories">
                                <SaltRimRadio
                                    ...
                                    :title="price.formattedTotals.total"
                    ...
                    <div v-else class="billing__card billing--active">
                        ...
                        <template v-if="billing.subscription">
                            <div v-if="billing.subscription.next_billed_at">
                                {{ t("billing.subscribed-on") }} <DateFormatter :date="billing.subscription.created_at" /> &middot;
                                {{ billing.customer.paddle_email }}
                                <br />
                                {{ t("billing.next_bill_at") }} <DateFormatter :date="billing.subscription.next_billed_at.date" /> ({{
                                    billing.subscription.next_billed_at.amount
                                }})
                            </div>
                            <div v-if="billing.subscription.paused_at">{{ t("billing.end_at") }} <DateFormatter :date="billing.subscription.paused_at" /></div>
                            <div v-if="billing.subscription.ends_at">{{ t("billing.end_at") }} <DateFormatter :date="billing.subscription.ends_at" /></div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="tx in billing.subscription.transactions" :key="tx.invoice_number">
                                ...
                                <td>
                                    {{ new Intl.NumberFormat(userLocale, { style: "currency", currency: tx.currency }).format(parseFloat(tx.total) / 100) }}
                                </td>
                                <td>{{ tx.currency }}</td>
                                <td>
...
function upgradePlan() {
    ...
    paddle.value.Checkout.open({
        items: [{ priceId: selectedPriceCategory.value }],
        customer: customer,
    });
}
