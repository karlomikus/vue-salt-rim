...
const { t } = useI18n();
const { cocktailIngredient, scaleFactor = 1, units = "ml", shoppingList = [] } = defineProps<Props>();
...
<template>
    <div>
        <div class="cocktail-ingredients__ingredient">
            ...
            <RouterLink
                class="cocktail-ingredients__ingredient__name"
                :to="{ name: 'ingredients.show', params: { id: cocktailIngredient.ingredient.slug } }"
                data-ingredient="preferred"
            >
                {{ cocktailIngredient.ingredient.name }}
                <span v-if="cocktailIngredient.note" class="cocktail-ingredients__flags__flag">&ndash; {{ cocktailIngredient.note }}</span>
                <small v-if="cocktailIngredient.optional">({{ t("optional") }})</small>
            </RouterLink>
        ...
        <div class="cocktail-ingredients__flags">
            <div v-if="cocktailIngredient.substitutes && cocktailIngredient.substitutes.length > 0" class="cocktail-ingredients__flags__flag">
                ...
                <template v-for="(sub, index) in cocktailIngredient.substitutes" :key="index">
                    <RouterLink
                        :style="{ 'font-weight': sub.in_bar_shelf ? 'bold' : 'normal' }"
                        :to="{ name: 'ingredients.show', params: { id: sub.ingredient.slug } }"
                        data-ingredient="substitute"
                    >
            ...
            <div v-if="shoppingList.map((i) => i.ingredient.id).includes(cocktailIngredient.ingredient.id)" class="cocktail-ingredients__flags__flag">
                &middot; {{ t("ingredient.on-shopping-list") }}
            </div>
