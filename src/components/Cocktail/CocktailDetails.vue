...
const printUrl = computed(() => {
    return router.resolve({
        name: "print.cocktail",
        params: { id: cocktail.value.slug },
        query: {
            scaleFactor: ingredientScaleFactor.value.toFixed(4),
            targetVolumeToScaleTo: targetVolumeToScaleTo.value,
            targetVolumeDilution: targetVolumeDilution.value,
    ...
    return (
        cocktail.value?.ingredients
            ?.filter((cocktailIngredient) => {
                return !cocktailIngredient.in_bar_shelf && !userShoppingListIngredients.value.map((i) => i.ingredient.id).includes(cocktailIngredient.ingredient.id);
            })
...
async function deleteCocktail() {
    confirm.show(t("cocktail.confirm-delete", { name: cocktail.value.name }), {
        onResolved: (dialog: any) => {
            dialog.close();
            BarAssistantClient.deleteCocktail(cocktail.value.id)
                .then(() => {
...
async function copy() {
    confirm.show(t("cocktail.confirm-copy", { name: cocktail.value.name }), {
        onResolved: (dialog: any) => {
            dialog.close();
            isLoading.value = true;
            BarAssistantClient.copyCocktail(cocktail.value.slug)
                .then((id) => {
                    isLoading.value = false;
...
function favorite() {
    isLoadingFavorite.value = true;
    BarAssistantClient.favoriteCocktail(cocktail.value.id)
        .then((resp) => {
...
async function addMissingIngredients() {
    ...
    isLoading.value = true;
    BarAssistantClient.addToShoppingList(appState.user.id, postData)
        .then(async () => {
    ...
    <div v-else>
        <PageHeader>
            ...
            <small>
                <template v-if="cocktail.parent_cocktail">
                    ...
                    <RouterLink :to="{ name: 'cocktails.show', params: { id: cocktail.parent_cocktail.slug } }">{{ cocktail.parent_cocktail.name }}</RouterLink>
                    &middot;
                </template>
                <DateFormatter :date="cocktail.created_at" format="short" />
                <template v-if="cocktail?.updated_user">&middot; {{ cocktail.updated_user.name }}</template>
            </small>
        </PageHeader>
        <article class="cocktail-details">
                ...
                <div class="cocktail-details__graphic">
                    <swiper-container v-if="cocktail.images && cocktail.images.length > 0" navigation="true" :pagination="{ clickable: true }" follow-finger="false">
                        <swiper-slide v-for="image in sortedImages" :key="image.sort">
                            ...
                            <div v-if="image.copyright" class="cocktail-details__graphic__copyright">
                                {{ t("imageupload.copyright-notice", { copyright: image.copyright }) }}
                            </div>
                        </swiper-slide>
                    </swiper-container>
            ...
            <div class="cocktail-details__column-sidebar">
                ...
                <CocktailCollections
                    :cocktail="cocktail"
                    @cocktail-removed-from-collection="fetchCocktail"
                    @add-to-collection="showCollectionDialog = !showCollectionDialog"
                ></CocktailCollections>
            ...
            <div class="cocktail-details__column-content">
                    ...
                    <Dropdown>
                        ...
                        <template #content>
                            ...
                            <SaltRimDialog v-model="showCollectionDialog">
                                ...
                                <template #dialog>
                                    <CollectionDialog
                                        :title="t('collections.add-to')"
                                        :cocktails="[cocktail.id]"
                                        @collection-dialog-closed="
                    ...
                    <div v-if="cocktail.ingredients && cocktail.ingredients.length > 0" class="block-container block-container--padded">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; margin-bottom: 1rem">
                            <div class="button-group">
                                ...
                                <button type="button" @click="showScaler = !showScaler">
                                    {{ $t("scale.scaled") }}: x{{ unitHandler.toFixedWithTruncate(ingredientScaleFactor, 2) }}
                                </button>
                            </div>
                            <UnitPicker></UnitPicker>
                        </div>
                        <CocktailRecipeScaler
                            class="volume-scaling-container"
                            v-show="showScaler"
                            v-model="ingredientScaleFactor"
                            v-model:waterDilution="waterDilution"
                            v-model:targetVolume="targetVolumeToScaleTo"
                            :cocktail-volume-ml="cocktail.volume_ml ?? 0"
                            :method-dilution="targetVolumeDilution"
                        ...
                        <ul class="cocktail-ingredients">
                            <li v-for="ing in cocktail.ingredients" :key="ing.sort">
                                <CocktailIngredientView
                                    ...
                                    :shopping-list="userShoppingListIngredients"
                                    :scale-factor="ingredientScaleFactor"
                                    :units="currentUnit"
                                ></CocktailIngredientView>
                            </li>
                        </ul>
                ...
                <div class="block-container block-container--padded has-markdown">
                    ...
                    <div v-if="cocktail.utensils && cocktail.utensils.length > 0">
                        ...
                        >: {{ cocktail.utensils.map((u) => u.name).join(", ") }}
                    </div>
                </div>
        ...
        <SaltRimDialog v-model="showManualCopyDialog">
            ...
            <template #dialog>
                ...
                <textarea class="form-input" :value="manualCopyText" readonly autofocus style="height: 300px"></textarea>
            </template>
        </SaltRimDialog>
    </div>
</template>
