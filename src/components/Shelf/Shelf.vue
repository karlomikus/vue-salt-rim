...
<template>
  ...
  <div v-else class="empty-state">
    ...
    <p>All cocktails you favorited are going to be right here.<br><RouterLink :to="{name: 'cocktails'}">Find cocktails</RouterLink></p>
  </div>
  <h2 class="page-subtitle">Cocktails you can make</h2>
  <cocktail-grid-container v-if="shelfCocktails.length > 0" v-slot="observer">
    <SingleCocktailItem v-for="cocktail in shelfCocktails" :cocktail="cocktail" :key="cocktail.id" :observer="observer" />
  </cocktail-grid-container>
  <div v-else class="empty-state">
    ...
    <p>You don't have enough ingredients to create any of the available cocktails.<br><RouterLink :to="{name: 'ingredients'}">Add ingredients</RouterLink></p>
  </div>
