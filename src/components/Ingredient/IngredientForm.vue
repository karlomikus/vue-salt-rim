<template>
    ...
    <form @submit.prevent="submit">
        ...
        <div class="block-container block-container--padded">
            <div class="form-group">
                ...
                <div class="form-group-ai" v-if="ingredient.name && ingredient.name.length >= 3">
                    <ButtonGenerate
                        :callFn="generateBasicIngredientInfo"
                        @before-generation="isLoadingGen = true"
                        @after-generation="isLoadingGen = false"
                        title="Generate basic ingredient information"
                    ></ButtonGenerate>
                </div>
            </div>
            <div class="form-group">
        ...
        <div class="block-container block-container--padded">
            <div class="form-group">
                <SaltRimCheckbox
                    id="parent-ingredient-checkbox"
                    v-model="isParent"
                    :label="$t('ingredient.is-variety')"
                    :description="$t('ingredient.variety-note')"
                ></SaltRimCheckbox>
            </div>
            <div v-show="isParent" class="form-group" v-if="bar.search_host">
                <IngredientFinderBasic
                    v-if="shouldUseBasicSearch"
                    v-show="ingredient.hierarchy?.parent_ingredient?.id == null"
                    @ingredient-selected="selectParentIngredient"
                    :disabled-ingredients="disabledFinderIngredients"
                ></IngredientFinderBasic>
                <IngredientFinder
            ...
            <div class="form-group">
                <SaltRimCheckbox
                    id="complex-ingredient-checkbox"
                    v-model="isComplex"
                    :label="$t('ingredient.is-complex')"
                    :description="$t('ingredient.complex-note')"
                ></SaltRimCheckbox>
            </div>
            <div v-show="isComplex" class="ingredient-form__complex-ingredients" v-if="bar.search_host && ingredient.ingredient_parts">
                <div>
                    <IngredientFinderBasic
                        v-if="shouldUseBasicSearch"
                        :selected-ingredients="ingredient.ingredient_parts.map((i) => i.ingredient.id)"
                        @ingredient-selected="selectIngredientPart"
                        :disabled-ingredients="disabledFinderIngredients"
                    ></IngredientFinderBasic>
                    <IngredientFinder
                        v-else-if="!shouldUseBasicSearch && appState.bar.search_token"
            ...
            <template v-if="priceCategories.length > 0">
                ...
                <div v-for="(price, idx) in ingredient.prices" :key="idx" class="block-container ingredient-prices__price">
                    ...
                    <div class="form-group" style="max-width: 150px">
                        <label class="form-label form-label--required" :for="'ingredient-price-price-' + idx"
                            >{{ $t("price.price") }}
                            <template v-if="price.price_category.id">({{ priceCategories.find((p) => p.id == price.price_category.id)?.currency }})</template></label
                        >
...
async function submit() {
    ...
    const postData = {
        ...
        complex_ingredient_parts: ingredient.value.ingredient_parts
            ...
            : [],
        prices: ingredient.value.prices
            ? ingredient.value.prices
                  .filter((p) => p.price_category.id != null)
                  .map((p) => ({
    ...
    if (ingredient.value.id) {
        BarAssistantClient.updateIngredient(ingredient.value.id, postData)
            .then(() => {
        ...
        BarAssistantClient.saveIngredient(postData)
            .then((newId) => {
