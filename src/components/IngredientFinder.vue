        ...
        <ais-search-box autofocus>
            <template #default="{ refine }">
                <input
                    ...
                    class="form-input ingredient-finder__search-input"
                    type="search"
                    :placeholder="t('placeholder.search-ingredients')"
                    autocomplete="off"
                    autocorrect="off"
                    autocapitalize="off"
                    spellcheck="false"
                    maxlength="512"
                    @input="refine(currentQuery)"
                />
            </template>
        </ais-search-box>
            ...
            <template #default="{ items }: { items: SearchResult[] }">
                <div class="ingredient-finder__options block-container block-container--inset">
                    ...
                    <a
                        v-for="item in items"
                        :key="item.id"
                        class="block-container block-container--hover"
                        href="#"
                        @click.prevent="selectIngredient(item)"
                        :class="{ 'ingredient-finder__options--disabled': disabledIngredients.includes(item.id) }"
                    >
...
const { searchToken, selectedIngredients = [], initialQuery = null, maxHits = 15, disabledIngredients = [] } = defineProps<Props>();
...
const searchClient = instantMeiliSearch(appState.bar.search_host ?? "", searchToken).searchClient;
